...
from avant import __version__ as ps_version
...
from avant.worker.database import (
    backup,
    discard_old_package_configs,
    discard_old_srpm_build_logs,
)
from avant.worker.handlers import (
    CoprBuildEndHandler,
    CoprBuildHandler,
...
from avant.worker.handlers.abstract import AvantTaskName
from avant.worker.helpers.build.babysit import (
    check_copr_build,
    check_pending_copr_builds,
    check_pending_testing_farm_runs,
)
from avant.worker.jobs import PackageJobs
...
def process_message(
    ...
    Returns:
        task results
    """
    return PackageJobs.process_message(event=event, source=source, event_type=event_type)
...
@celery_app.task(name=AvantTaskName.submit_package, base=TaskWithRetry)
...
@celery_app.task(name=AvantTaskName.copr_build_start, base=TaskWithRetry)
...
@celery_app.task(name=AvantTaskName.copr_build_end, base=TaskWithRetry)
...
@celery_app.task(bind=True, name=AvantTaskName.copr_build, base=TaskWithRetry, queue="long-running")
...
@celery_app.task(bind=True, name=AvantTaskName.testing_farm, base=TaskWithRetry)
...
@celery_app.task(name=AvantTaskName.testing_farm_results, base=TaskWithRetry)
