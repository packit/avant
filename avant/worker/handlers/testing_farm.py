...
from avant.worker.utils import elapsed_seconds
from avant.worker.checker.abstract import Checker
from avant.worker.checker.testing_farm import (
    CanActorRunJob,
    IsCoprBuildDefined,
    IsEventForJob,
    IsEventOk,
...
from avant.worker.handlers import JobHandler
...
from avant.worker.helpers.testing_farm import (
...
class TestingFarmHandler(
    ...
    task_name = AvantTaskName.testing_farm
...
class TestingFarmResultsHandler(
    ...
    __test__ = False
    task_name = AvantTaskName.testing_farm_results
    ...
    def run(self) -> TaskResults:
        ...
        if self.result == TestingFarmResult.running:
            logger.info(f"Testing farm {self.pipeline_id} result:\n{self.result}")
        else:
            ...
            logger.info(
                f"Testing farm {self.pipeline_id} ran for {test_run_time} result:\n{self.result}"
            )
