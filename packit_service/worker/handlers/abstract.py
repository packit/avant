...
class Handler(PackitAPIProtocol, Config):
    ...
    def run_n_clean(self) -> TaskResults:
        try:
            ...
            if self.service_config.deployment in [Deployment.stg, Deployment.dev]:
                ...
                logger.info(
                    f"Objects still in memory generated by task execution: {leftover}")
            self.clean()
    ...
    def _clean_workplace(self):
        ...
        if not p.is_dir():
            logger.debug(
                f"Directory {
                    self.service_config.command_handler_work_dir!r} does not exist.",
            )
            return
